// https://codepen.io/rileyjshaw/pen/eZRLaP
...
const GLITCH_PAUSE_DURATION = [1500, 1900]; // min, max in ms.
const COLOR = [55, 255, 55];
const TITLE = 'MiniMap.AI';
...
function drawStatic() {
  ...
  const fade = ctx.createLinearGradient(0, 0, 0, y0 / .2);
  ...
  // Draw spaceship.
  ctx.beginPath();
  ctx.arc(w / 2, h / 2, SPACING_X * 1.5, Math.PI * 5 / 16, Math.PI * 11 / 16);
  ctx.lineTo(w / 2, h / 2);
  ctx.closePath();
  stroke(ctx);
  ...
  ctx.font = '64pt "IBM Plex Mono", monospace';
  ctx.fillText(TITLE, w / 2, y0 / 2);

  ctx.fillStyle = `rgba(${COLOR}, 1)`;
  ctx.font = '48pt "IBM Plex Mono", monospace';
  ctx.fillText(DESCRIPTION, w / 2, y0 / 1.15);
